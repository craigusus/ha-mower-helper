...
- **State reporting delay** — HA may detect the `mowing` state significantly later than the actual start. This affects `mower_started_at`, calendar event start times, and duration calculations in Discord notifications.
...
- **`sensor.mower_mow_days` transient `unavailable`/`unknown`** — On HA startup or a full config reload, template sensors can briefly evaluate before their source helpers are ready, so `sensor.mower_mow_days` may read as `unavailable` for one cycle. Every `| from_json` read of it (in `sensor.mower_next_scheduled`, its `iso` attribute, the scheduled-mow and retry automation conditions, and the calendar placeholder sync) now passes `[]` as a default (`| from_json([])`) so a bad read is treated as "no mow days" for that cycle instead of throwing a `TemplateError`.
